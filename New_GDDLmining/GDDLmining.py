...
def mainMethod(lattice, level0Set, dependency_set, literals, Rhs):
    compute_dependencies(level0Set, dependency_set, Rhs)
    for i in range(1, len(level0Set)):
        ...
        compute_dependencies(lattice[i], dependency_set, Rhs)
        end_time = time.time()
...
def compute_dependencies(levelSet, dependency_set, Rhs):
    for block in levelSet:
        reLiteralsSet = []
        for literal in block.literalsSet:
            lhsMatches = literal[0].l
            for lsh in literal:
                lhsMatches = tuple(set(lhsMatches).intersection(set(lsh.l)))
                if set(lhsMatches).issubset(set(Rhs.l)):
                    dependency = ""
                    for i in range(len(literal)):
                        if (i == len(literal) - 1):
                            dependency += literal[i].name + ' ' + str(literal[i].sigma)
                        else:
                            dependency += literal[i].name + ' ' + str(literal[i].sigma) + ";;"
                    dependency += "->"
                    dependency += Rhs.name + ' ' + str(Rhs.sigma)
        ...
        for line in f:
            if i == 0:
                ...
                for i in range(0, len(line)):
                    if ".id" in line[i] and line[i] not in id_set:
                        id_set.append(line[i])
                for i in range(0, len(id_set)):
                    id_position.append(line.index(id_set[i]))
                    # rhs_set[id_set[i]] = []
                i += 1
            else:
                aary = []
...
if __name__ == "__main__":
    ...
    print(lattice)
    dependency_set = []
    mainMethod(lattice, level0Set, dependency_set, literals, Rhs)
